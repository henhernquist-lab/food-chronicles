...
export function ArticleCard({ article, featured = false, delay = 0 }: { article: Article; featured?: boolean; delay?: number }) {
  ...
  return (
    <Link to={`/article/${article.slug}`} className={`group block article-card ${featured ? "featured-card" : ""}`} style={{ animationDelay: `${delay}ms` }}>
      ...
      <div className="mt-4 space-y-2 p-4 md:p-5">
